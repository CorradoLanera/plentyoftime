...
phone, and it was quite difficult to me to find R source code without R, any
suggestion?!]. There I have found a line of code which could possibly solve the
problem. I search for a __datacamp__-like WEBpage to run the code, and as soon
as a call to something like `identical(f(x), y)` returned me `TRUE` I posted my
solution:
...
and "%Y-%m-%d" otherwis__), it is parse to the midnight.
...
Hence, if we convert ithe first part in seconds, and we add, in our example,
...
only the `tzone` attributes of the final object but also it "real" value, in 
the measure provided in the `gmtoff`. Moreover, note that the `isdst` has no
influence on the value but ony on the representation of the object
...
about the stored timezone. So, we can try to apply this function a see what
happen
